...
using System.Text;
using Traveller.Interface;
using Traveller.Models;
using Traveller.Models.DTO;
using Traveller.Models.Helpers;

namespace Traveller.Service
{
    public class AgentService :IAgentService,IAgent
    {
        ...
        public async Task<string> Login(Auth_DTO auth_DTO)
        {
            if (auth_DTO != null && !string.IsNullOrEmpty(auth_DTO.email_id) && !string.IsNullOrEmpty(auth_DTO.password))
            {
                ...
                if (user != null && user.status == "activate" && PasswordHasher.VerifyPassword(auth_DTO.password, user.password))
                {
                    var claims = new[]
                {
